...
//          bitmap->pixel_data[i * bitmap->width + j].green = z;
            ...
            bitmap->imageData[ i * step + j * nChannels +
                               2 ] =
                ( ( s[ 0 ] * xyzTorgb[ 0 ][ 0 ] + s[ 1 ] * xyzTorgb[ 0 ][ 1 ] + s[ 2 ] * xyzTorgb[ 0 ][ 2 ] ) * scale );
            bitmap->imageData[ i * step + j * nChannels +
                               1 ] =
                ( ( s[ 0 ] * xyzTorgb[ 1 ][ 0 ] + s[ 1 ] * xyzTorgb[ 1 ][ 1 ] + s[ 2 ] * xyzTorgb[ 1 ][ 2 ] ) * scale );
            bitmap->imageData[ i * step + j *
                               nChannels ] =
                ( ( s[ 0 ] * xyzTorgb[ 2 ][ 0 ] + s[ 1 ] * xyzTorgb[ 2 ][ 1 ] + s[ 2 ] * xyzTorgb[ 2 ][ 2 ] ) * scale );
        }
    }
...
//       << " , " << (int) bitmap->pixel_data[i * bitmap->width + j].blue;
            ...
            bitmap->imageData[ i * step + j * nChannels + 2 ] = scale *
                                                                ( s[ 0 ] * rgbToxyz[ 0 ][ 0 ] + s[ 1 ] *
                                                                  rgbToxyz[ 0 ][ 1 ] + s[ 2 ] * rgbToxyz[ 0 ][ 2 ] );
            bitmap->imageData[ i * step + j * nChannels + 1 ] = scale *
                                                                ( s[ 0 ] * rgbToxyz[ 1 ][ 0 ] + s[ 1 ] *
                                                                  rgbToxyz[ 1 ][ 1 ] + s[ 2 ] * rgbToxyz[ 1 ][ 2 ] );
            bitmap->imageData[ i * step + j * nChannels ] = scale *
                                                            ( s[ 0 ] * rgbToxyz[ 2 ][ 0 ] + s[ 1 ] *
                                                              rgbToxyz[ 2 ][ 1 ] + s[ 2 ] * rgbToxyz[ 2 ][ 2 ] );
        }
    }
...
//       << " , " << (int) bitmap->pixel_data[i * bitmap->width + j].blue;
            ...
            bitmap->imageData[ i * step + j * nChannels + 2 ] = scale *
                                                                ( s[ 0 ] * xyzTorgb[ 0 ][ 0 ] + s[ 1 ] *
                                                                  xyzTorgb[ 0 ][ 1 ] + s[ 2 ] * xyzTorgb[ 0 ][ 2 ] );
            bitmap->imageData[ i * step + j * nChannels + 1 ] = scale *
                                                                ( s[ 0 ] * xyzTorgb[ 1 ][ 0 ] + s[ 1 ] *
                                                                  xyzTorgb[ 1 ][ 1 ] + s[ 2 ] * xyzTorgb[ 1 ][ 2 ] );
            bitmap->imageData[ i * step + j * nChannels ] = scale *
                                                            ( s[ 0 ] * xyzTorgb[ 2 ][ 0 ] + s[ 1 ] *
                                                              xyzTorgb[ 2 ][ 1 ] + s[ 2 ] * xyzTorgb[ 2 ][ 2 ] );
        }
    }
...
void convertion( IplImage* bitmap, std::string conv, int step, int nChannels )
{
    ...
    else if( conv.compare( "luv2rgb" ) == 0 )
    {
        for( unsigned int i = 0; i < bitmap->height; i++ )
        {
            for( unsigned int j = 0; j < bitmap->width; j++ )
