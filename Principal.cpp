...
void inverte(IplImage* src, IplImage* dst) //inverte imagem na vertical para mostrar corretamente na tela e a ordem dos canais
{
    for(int i = 0 ; i < src->height ; i++ ) {
        for(int j = 0 ; j < src->width ; j++ ) {
            src->imageData[i*step + j*nChannels+2] = dst->imageData[(dst->height-i)*step + j*nChannels];
            src->imageData[i*step + j*nChannels+1] = dst->imageData[(dst->height-i)*step + j*nChannels+1];
            src->imageData[i*step + j*nChannels] = dst->imageData[(dst->height-i)*step + j*nChannels+2];
        }
    }
}
void inverte_volta() //retorna posição da imagem para salvar
{
    for(int i = 0 ; i < img->height ; i++ ) {
        for(int j = 0 ; j < img->width ; j++ ) {
            imgAux->imageData[i*step + j*nChannels+2] = img->imageData[(imgAux->height-i)*step + j*nChannels];
            imgAux->imageData[i*step + j*nChannels+1] = img->imageData[(imgAux->height-i)*step + j*nChannels+1];
            imgAux->imageData[i*step + j*nChannels] = img->imageData[(imgAux->height-i)*step + j*nChannels+2];
        }
    }
}
...
//Crescimento de região pelas fórmulas
void avalia_pixel(int x, int y){
    if (!(vflags[(y*step) + x]))
    {
        ...
        if (!luv_activated){
            ...
            if (!((R > 30) && (((G-84) < 00)||((G-84) > -20)) && ((-(B-125))>10) && ((-(B-125))<60) ))
            {
                img->imageData[(y*step) + x*nChannels] = 0;
                img->imageData[(y*step) + x*nChannels+1] = 255;
                img->imageData[(y*step) + x*nChannels+2] = 0;
...
//Pega os valores limites dos canais no retênculo selecionado
void calcula_min_max(IplImage* src){
    char* pixel;
    int t;
    int total = 0;
    pixel = src->imageData;
//    int maxL, minL, maxU, minU, maxV, minV;
    maxL = maxU = maxV = -INFINITY;
    minL = minU = minV = INFINITY;
    for (int i = ROI.x + 1; i < ROI.x + ROI.width; i++){  //+1 pra não pegar borda do retangulo desenhado
       for (int j= ROI.y + 1; j < ROI.y + ROI.height; j++){
           ...
           if ((unsigned char) pixel[t]   > maxL) maxL = (unsigned char) pixel[t];
           if ((unsigned char) pixel[t+1] > maxU) maxU = (unsigned char) pixel[t+1];
           if ((unsigned char) pixel[t+2] > maxV) maxV = (unsigned char) pixel[t+2];
           if ((unsigned char) pixel[t]   < minL) minL = (unsigned char) pixel[t];
           if ((unsigned char) pixel[t+1] < minU) minU = (unsigned char) pixel[t+1];
           if ((unsigned char) pixel[t+2] < minV) minV = (unsigned char) pixel[t+2];
           //printf("total = %d\n", total);
           //pixel[t] = 255;
           total++;
        }
...
void calcula_media(IplImage* src){
    char* pixel;
    int total = 0;
    int t;
    pixel = src->imageData;
...
//Aplica limites na imagem
void aplica_formula(IplImage *src){
...
//calcula porcentagem de acerto e erro
void compara_groundtruth(){
    ...
    for(int i = 0 ; i < img->width ; i++ ) {
        for(int j = 0 ; j < img->height ; j++ ) {
            sub = (unsigned char)img_luv->imageData[j*step + i*nChannels] - (unsigned char)img_gt->imageData[j*step + i*nChannels];
...
// Função que detecta olhos e face
void detectEyes(IplImage *src, char* spaceColor, IplImage *src_luv)
{
	int i;
	int fcenter_x = 0;
	int fcenter_y = 0;
...
//	}
    ...
    if (spaceColor == "luv"){
        calcula_min_max(src_luv);
        aplica_formula(src_luv);
    } else {
        calcula_min_max(src);
        aplica_formula(src);
    }
...
//Retangulo para seleção da área desejada
void desenhaRetangulo(int init_x, int init_y, int x, int y)
{
    cvResetImageROI(img);
...
//    ROI = cvRect(init_x, init_y + desl_y, abs(desl_x), abs(desl_y));
    ...
    stateGlobal = state;
      ...
      if (button == GLUT_LEFT_BUTTON){
           if(state == GLUT_DOWN){
             ...
             printf("Valores do Pixel - R: %d - G: %d - B: %d\n", (uchar) img->imageData[(mouseY*step) + mouseX*nChannels], (uchar) img->imageData[(mouseY*step) + mouseX*nChannels+1], (uchar) img->imageData[(mouseY*step) + mouseX*nChannels+2]);
             printf("Valores do Pixel - L: %d - U: %d - V: %d\n", (uchar) img->imageData[(mouseY*step) + mouseX*nChannels], (uchar) img->imageData[(mouseY*step) + mouseX*nChannels+1] - 84, (uchar) img->imageData[(mouseY*step) + mouseX*nChannels+2] -125);
...
//    stateGlobal = state;
      ...
      if (button == GLUT_LEFT_BUTTON){
           if(state == GLUT_DOWN){
             printf("Ground Truth: Valores do Pixel - R: %d - G: %d - B: %d\n", (uchar) img_gt->imageData[(mouseY_gt*step) + mouseX_gt*nChannels], (uchar) img_gt->imageData[(mouseY_gt*step) + mouseX_gt*nChannels+1], (uchar) img_gt->imageData[(mouseY_gt*step) + mouseX_gt*nChannels+2]);
           }
       }
...
void mouseMotion(int x, int y){
  ...
  if (buttonGlobal == GLUT_LEFT_BUTTON){
       if(stateGlobal == GLUT_DOWN){
           desenhaRetangulo(init_x, init_y, desl_x,desl_y);
       }
  }
   ...
   switch (key){
    ...
    case 'a':
       for (int i = 0; i<img->width; i++){
        ...
        break;
    case 'd':
//        inverte(imgAux, img_rgb);
//        inverte(tempData, img_luv);
//        detectEyes(imgAux); // detecta pelo img_rgb (imagem original) e modifica na que esta na tela
//        inverte(img_rgb, imgAux);
//        inverte(img_luv, tempData);


//        inverte(imgAux, img_rgb);
//        inverte(tempData, img);
//        detectEyes(tempData); // detecta pelo img_rgb (imagem original) e modifica na que esta na tela
//        inverte(img_rgb, imgAux);
//        inverte(img, tempData);

        cvCopyImage(img_rgb, tempData);
        inverte(imgAux, img_luv);
        detectEyes(tempData, "luv", imgAux); // detecta pelo img_rgb (imagem original) e modifica na que esta na tela
        inverte(img_luv, imgAux);

        cvCopyImage(img_rgb, tempData);
        detectEyes(tempData,"rgb", NULL); // detecta pelo img_rgb (imagem original) e modifica na que esta na tela
        inverte(img, tempData);
    ...
    case 'f':
        ...
        calcula_min_max(img_luv);
        aplica_formula(img_luv);
        break;
    case 'm':
        calcula_media(img);
        aplica_formula(img);

        calcula_media(img_luv);
        aplica_formula(img_luv);
        break;
    case 'c':
        compara_groundtruth();
...
int main(int argc, char *argv[]){
    ...
    img = cvLoadImage("soldado.bmp", CV_LOAD_IMAGE_COLOR );
    img_gt = cvLoadImage("soldado.bmp", CV_LOAD_IMAGE_COLOR );
//    img = cvLoadImage("pele7.png", CV_LOAD_IMAGE_COLOR );
//    img_gt = cvLoadImage("pele7.png", CV_LOAD_IMAGE_COLOR );

    //img = cvLoadImage("pele5.png", CV_LOAD_IMAGE_COLOR );
...
//    img = cvLoadImage("teste.png", CV_LOAD_IMAGE_COLOR );
    ...
    printf("|Trabalho Processamento de Imagens    |\n");
    printf("|Rafael de Souza Marques              |\n");
    printf("|Joao Paulo Scoralick                 |\n");
    printf("|Marco Aurelio Goncalves da Silva     |\n");
    printf("|_____________________________________|\n\n");
    printf("Clique na imagem para marcar\n");
    printf("Pressione 's' para salvar a imagem\n");

    glutInitWindowSize(img->width,img->height);
    glutInitWindowPosition(500,100);
    glutInitDisplayMode(GLUT_RGB | GLUT_DOUBLE | GLUT_DEPTH);

    glutCreateWindow("Trabalho de Processamento de Imagens");
